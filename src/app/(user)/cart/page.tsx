  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={clearCart}
          className="text-sm text-[#ba1a1a] hover:underline transition-colors"
        >
          Clear all
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-[#1b1b1d] font-[family-name:var(--font-geist)] text-sm leading-snug line-clamp-2 mb-1">
                  {item.title}
                </p>
                <p className="text-[#006c49] font-bold font-[family-name:var(--font-geist)] text-base">
              ...
              <div className="flex flex-col items-end justify-between gap-2">
                ...
                <div className="flex items-center border border-[#c6c6cd] rounded-xl overflow-hidden">
                  <button
                    ...
                    className="px-2.5 py-2 text-[#45464d] hover:bg-[#f0edef] transition-colors disabled:opacity-40"
                    aria-label="Decrease"
                  >
                    <Minus size={12} />
                  </button>
                  <span className="w-8 text-center text-sm font-medium text-[#1b1b1d] font-[family-name:var(--font-geist)]">
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => updateQuantity(item.productId, item.quantity + 1)}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl shadow-[0px_4px_20px_rgba(15,23,42,0.05)] p-6 sticky top-24">
            ...
            <div className="space-y-3 mb-5 pb-5 border-b border-[#f0edef]">
              {items.map((item) => (
                <div key={item.productId} className="flex justify-between text-sm">
                  <span className="text-[#45464d] line-clamp-1 flex-1 mr-2">
                    {item.title} × {item.quantity}
                  </span>
                  <span className="font-medium text-[#1b1b1d] flex-shrink-0">
                    {formatPrice(item.price * item.quantity)}
                  </span>
                </div>
              ))}
            </div>
            <div className="flex justify-between text-base font-bold text-[#0f172a] font-[family-name:var(--font-geist)] mb-6">
              <span>Total</span>
            ...
            <Link
              href="/products"
              className="block text-center text-sm text-[#76777d] hover:text-[#0f172a] mt-3 transition-colors"
            >
              ← Continue Shopping
            </Link>
          </div>
